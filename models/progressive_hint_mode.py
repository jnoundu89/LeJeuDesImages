...
class ProgressiveHintMode(GameMode):
    ...
    def get_question_data(self, data_id: int, used_indices: List[int], 
        ...
        hints = [
            {
                'level': 1,
                'text': f"Cette personne travaille chez {selected_employee['company']}",
                'points': 3  # Maximum points if guessed with just this hint
            },
            {
                'level': 2,
                'text': f"Cette personne fait partie de l'équipe {selected_employee['team']}",
                'points': 2  # Points if guessed with this hint
            },
            {
                'level': 3,
                'text': f"Cette personne occupe le poste de {selected_employee['position']}",
