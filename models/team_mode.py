...
class TeamMode(GameMode):
    ...
    def get_question_data(self, data_id: int, used_indices: List[int], 
        ...
        all_names = [member.get('name', '') for member in self.game_manager.employee_data.get_all_employees()]
        correct_names = [member.get('name', '') for member in selected_members]
        
        # Create choices for each member
        member_choices = []
        for member in selected_members:
            correct_name = member.get('name', '')
