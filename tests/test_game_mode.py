...
class TestGetNameChoices:
    ...
    def test_falls_back_when_sex_filter_yields_too_few(self, stub_mode):
        """Regression: when the dataset's `sex` column is malformed
        (e.g. mapped to a unique-per-row column like an id, or full of
        free-text values), the sex filter narrows the pool to 0 and the
        question used to render with only 1 choice (the answer). With
        the fallback in place, we draw distractors from the full pool."""
        employees = stub_mode.game_manager.employee_data.get_all_employees()
        # Synthesise a row with a sex value no one else shares so the
        # `_exclude_self` filtered pool comes back empty.
        selected = dict(employees[0])
        selected['sex'] = '__unique_sex__'
        choices = stub_mode._get_name_choices(selected, count=4)
        # Still 4 distinct names — fallback to the full employee pool.
        assert len(choices) == 4
