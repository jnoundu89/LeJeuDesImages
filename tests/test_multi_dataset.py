...
class TestHeaderDropdown:
    def test_dropdown_rendered_with_multiple_datasets(self, tmp_path):
        ...
        resp = client.get('/about')
        assert b'dataset-switcher' in resp.data
    ...
    def test_dropdown_not_rendered_with_single_dataset(self, app):
        # Default fixture app loads legacy test_config.yaml → single 'default' dataset
        client = app.test_client()
        resp = client.get('/about')
        # dataset-switcher block should be absent (only 1 dataset)
        assert b'dataset-switcher' not in resp.data
